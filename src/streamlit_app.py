...
from src.cached_resources import get_guard, instrument # ADD IMPORT

from gptcache import Cache
from gptcache.adapter.api import get, put #ADD IMPORT 
...
def generate_response(input_text: str, cache: Cache, guard: gd.Guard) -> None:
    ...
    try:
        ...
        # Use get()/put() GPTCache API for greater control
        cached_result = get(
            input_text,
            cache_obj=cache,
            top_k=GPTCACHE_GET_TOP_K, # top_k=1 as we want the first result
        )
        # Cache miss
...
def main() -> None:
    ...
    with st.form("my_form"):
        ...
        if submitted:
            generate_response(text, cache, guard) # Pass cache in
